...
export default function RegisterPage() {
  const [formData, setFormData] = useState({
    ...
    password: '',
    confirmPassword: '',
    role: 'CHAUFFEUR', // Valeur par défaut
    CIN: '',
    telephone: ''
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        router.push('/'); // ← Changement ici : '/' au lieu de '/login?registration=success'
      } else {
        const errorData = await response.json();
        setErrors({ ...errors, form: errorData.message || 'Erreur lors de l\'inscription' });
      }
    } catch (error) {
      setErrors({ ...errors, form: 'Erreur réseau' });
    } finally {
